...
- Stale deals = no activity 30+ days, Stuck = same stage 60+ days`;
  ...
  fallbackPatternMatching(userMessage, conversationContext) {
    ...
    // NEW: "What do we know about [Company]?" - Account context queries
    if (message.match(/what do we know about|tell me about|info about|information about|context (?:on|about|for)|show me.*about/i)) {
      intent = 'account_context';
      
      const companyMatch = message.match(/(?:about|on|for)\s+(.+?)(?:\?|$)/i);
      ...
      return {
        ...
        followUp: false,
        confidence: 0.9,
        explanation: 'Account context query - comprehensive account information',
        originalMessage: userMessage,
        timestamp: Date.now()
      };
    }
    ...
    if (message.match(/(?:show|what|list).*accounts.*(?:does|for|by)|(?:\w+)'s accounts/i)) {
      intent = 'owner_accounts';
      
      // Extract owner name
      const ownerMatch = message.match(/(\w+)'s accounts/i) ||
                        message.match(/accounts.*?(?:does|for|by)\s+(\w+)/i) ||
                        message.match(/(\w+).*accounts/i);
      ...
      return {
        ...
        followUp: false,
        confidence: 0.9,
        explanation: 'List accounts owned by specific person',
        originalMessage: userMessage,
        timestamp: Date.now()
      };
    }
    ...
    if (message.match(/what.*opportunities|show.*opportunities|(?:\w+).*opps/i) && !message.includes('many')) {
      intent = 'account_opportunities';
      
      const companyMatch = message.match(/(?:opportunities|opps).*?(?:does|for|at)\s+(.+?)(?:\s+have|\?|$)/i) ||
                          message.match(/(?:does|at)\s+(.+?)\s+(?:have|opps|opportunities)/i) ||
                          message.match(/(.+?)'?s?\s+(?:opportunities|opps)/i);
    ...
    // NEW: "Show me [Company]'s meeting history" or "when was our last meeting with [Company]?"
    if (message.match(/meeting.*history|last meeting|recent meeting|when.*meet/i)) {
      intent = 'account_meetings';
      
      const companyMatch = message.match(/(?:with|for|at)\s+(.+?)(?:'s|\?|$)/i) ||
                          message.match(/(.+?)'s?\s+meeting/i);
      ...
      return {
        ...
        followUp: false,
        confidence: 0.9,
        explanation: 'Show meeting history for account',
        originalMessage: userMessage,
        timestamp: Date.now()
      };
    }
